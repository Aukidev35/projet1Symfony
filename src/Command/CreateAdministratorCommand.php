...
class CreateAdministratorCommand extends Command
{
    ...
    {
        parent::__construct('app:create-admionistrator');
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$email) {
            $question = new Question('Quel est son email :');
            $email = $helper->ask($input, $output, $question);
        }
        $plainPassword = $input->getArgument('password');
        if (!$plainPassword) {
            $question = new Question('Quel est son mot de pasee :');
        ...
        $io->success('Le nouvelle administrateur à été crée.');
